...
while (WiFi.status() != WL_CONNECTED) {//inicio conecion 
delay(1500);// demora poara volver a intentar la conexion 
}
client.setServer(mqtt_server, mqtt_port); //estableco conexion al server mwtt 
client.setCallback(callback);// inicio el calback de server mqtt y espero datos
}
...
void displayInfo() { // rutina de captura del dato por GPS
if (gps.location.isValid()) {//verifico que el datro sea valido del objeto gps
...
client.publish(pubTopic, mqtt_payload);// publico en el brojer el dato del gps
